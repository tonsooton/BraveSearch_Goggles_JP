...
* 利用方法
[[https://search.brave.com/goggles/discover][Goggleの検索画面]]から各Goggleの名前を検索し、項目の右下の+ボタンを押して追加してください。


* 各Goggleについての解説

** abehiroshi.goggle
90~00年代に流行した、いわゆる「[[http://abehiroshi.la.coocan.jp/][阿部寛のホームページ]]」のようなウェブサイトのみを表示させます。日本国内のISPやレンタルサーバーサービスが提供しているWebスペースのURLを記載しています。
...
- ユーザー名を指すサブディレクトリ名が~(チルダ)などで区別されていない場合、Webスペースの提供元の公式ホームページが表示される場合がございます。


*** japanese_blog_services.goggle
- ブログサービスを利用している5ちゃんねる系まとめサイトが表示される場合があります。ノイズに思われる方はブラウザ拡張機能の[[https://iorate.github.io/ublacklist/ja/docs/getting-started][ublacklist]]をご活用ください。
